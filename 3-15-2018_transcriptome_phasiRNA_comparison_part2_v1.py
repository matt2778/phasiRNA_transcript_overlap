...
#   This version will include phasiRNA loci overlapping with transcript start site
...
with open (input_transcriptome_txt) as transLoc:
    for line in transLoc:
        ...
        bar = df.loc[(df['chromosome']==chrom) & (df['phasiStart']<=transStart) & (df['phasiStop']>=transStart)]
        if bar.empty:
            continue
        else:
            foo = foo + line
